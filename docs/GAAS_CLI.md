...
`realms seed` (Realms repo) deploys marketplace + file-registry via dfx, then registers those canister IDs on the same GaaS Casals conductor and runs `casals sheet deploy casals.json` (repo root). Sheet stands: `marketplace`, `file-registry` (no per-stand batons). DNS-mapped `marketplace_frontend` is adopt-only (`register_canister` + reconcile by name). The fleet `file_registry` backend is on the sheet and adopted by name — never minted a second time. `gaas new` uses `gos-as-a-service/casals.json` (governance, installer, realm-registry). There is no infra baton: batons are per-realm and minted with the realm by the installer (`orchestration-baton@1.3.0`), which is why that wasm still has to be authorized in the conductor.
