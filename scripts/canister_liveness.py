...
Used by the portal build (``src/realm_registry_frontend/scripts/assert-canister-live.js``)
before a frontend bake that would inject a portal URL. A missing principal
(IC0301 / HTTP 404) must not be baked. Known-dead IDs (fdr7z, installer
ghosts, …) are rejected without a network call.
...
def _dfx_canister_status(canister_id: str, network: str) -> tuple[int, object]:
    """``dfx canister status`` as a liveness probe: 200 when it answers, else 404."""
    env = {**os.environ, "TERM": "xterm", "NO_COLOR": "1", "DFX_WARNING": "-mainnet_plaintext_identity"}
    try:
        ...
        return 404, {"error": f"IC0301 canister not found (dfx not found: {exc})"}
    if result.returncode != 0:
        detail = (result.stderr.strip() or result.stdout.strip())[-500:]
        return 404, {"error": f"IC0301 canister not found ({detail})"}
    status = "unknown"
    for line in result.stdout.splitlines():
        if line.lower().startswith("status:"):
            status = line.split(":", 1)[1].strip().lower()
            break
    return 200, {"canister_id": canister_id, "status": status}
...
def fetch_canister_record_via_replica(
    ...
    *,
    ...
    return _dfx_canister_status(canister_id, network)
...
def fetch_local_canister_record(canister_id: str) -> tuple[int, object]:
    ...
    A local replica often reports a missing principal as a generic status
    failure rather than IC0301; any status error fails closed.
    """
    return _dfx_canister_status(canister_id, "local")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args or args[0] in ("-h", "--help"):
        print(
            "usage: python3 scripts/canister_liveness.py <canister_id> [role]\n"
            "       python3 scripts/canister_liveness.py --probe-baked [repo_root]",
            file=sys.stderr,
        )
        return 2
    try:
