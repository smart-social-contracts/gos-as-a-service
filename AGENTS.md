...
The environment is declared in `casals.json` and built with `casals up` (see `docs/OPERATIONS.md` and the Casals repo). Production is three commands from the Casals and realms checkouts: `casals up`, then `realms domains apply`, then `realms files publish`. The identity is a short-lived `icp identity delegation` from `prod-identity` — one HSM touch for the run (`Casals/docs/OPERATIONS.md`, "Hardware keys"). A laptop uses `realms/scripts/local_up.sh --gaas`.
...
There is one path per environment: `casals up -e <env>` on `casals.json`,
then publish (`docs/OPERATIONS.md`). The sheet installs `release:` artifacts.
`local` is a fresh replica; `production` is `gos.earth`. The
